...
class ProtectedZones(Base):
    ...
    __table_args__ = (
        PrimaryKeyConstraint('name', name='protected_zones_pkey'),
    )
...
class Alerts(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(['zone_name'], ['protected_zones.name'], name='alerts_zone_name_fkey'),
    ...
    zone: Mapped[Optional['ProtectedZones']] = relationship(
        'ProtectedZones',
        back_populates='alerts',
        primaryjoin='Alerts.zone_name == ProtectedZones.name',
        foreign_keys='Alerts.zone_name'
    )
